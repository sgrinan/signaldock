		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if got := parsedURL.String(); got != tt.want {
					t.Errorf("ParseURL(%q) = %q, want %q", tt.rawURL, got, tt.want)
				}
			})
		}
	})
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if !errors.Is(err, tt.wantErr) {
					t.Errorf("ParseURL(%q) error = %v, want %v", tt.rawURL, err, tt.wantErr)
				}

				if got != nil {
					t.Errorf("ParseURL(%q) = %v, want nil", tt.rawURL, got)
				}
			})
		}
	})
